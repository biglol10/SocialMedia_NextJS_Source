...
// post which is liked by user, userTonotifyId = the user whom we are going to notify
// user is from whom the notification is
...
const removeCommentNotification = async (
  ...
  try {
    ...
    // looking for the notification whose type is newLike and whose postId is equal to postId parameter and userId=userIdparameter
    const notificationToRemove = await user.notifications.find(
      (notification) =>
